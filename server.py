...
video = cv2.VideoCapture(0)
while (video.isOpened()):
    motorakim= [random.randint(10, 80), random.randint(10, 80), random.randint(10, 80), random.randint(10, 80), random.randint(10, 80), random.randint(10, 80)]
    ret, frame = video.read()
    frame = cv2.resize(frame, (500, 500), interpolation=cv2.INTER_CUBIC)
    encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 90]
    retval, buffer = cv2.imencode('.jpg', frame, encode_param)
    ...
    jpg_text = str(jpg_text).split("'")[1]
    gonderilcekData = '''
        {"motorlar":''' + str(motorakim) + ''',"sicaklik":''' + str(random.randint(10,80)) + ''',"resim":"''' + jpg_text + '''","basinc":''' + str(random.randint(10,80)) + ''',"basinc1":''' + str(random.randint(10,80)) +'''}'''
